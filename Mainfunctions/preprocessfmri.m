...
sliceorder        = paralist.sliceorder;
...
TemplatePath = '/lustre/iCAN/home/zhuangliping/scripts/spm8_scripts/BatchTemplates';
SPGRFolder = 'anatomy';
SPGRFileName = 'I';
...
%           
        ...
        case 'a'   % preprocessfmri_slicetime
          ...
          preprocessfmri_slicetime(WholePipeLine, TemplatePath, InputImgFile, FlipFlag, PfileDir, TempDir,sliceorder); % what is the difference?
    ...
    if strcmp(PrevPrefix(1), 's')
      ...
      unix(sprintf('gzip -fq %s', fullfile(OutputDir, 'meanI*')));
      unix(sprintf('gzip -fq %s', fullfile(TempDir, 'meanI*')));
      unix(sprintf('mv -f %s %s', fullfile(TempDir, 'meanI*'), OutputDir));
      unix(sprintf('mv -f %s %s', fullfile(TempDir, '*txt'), OutputDir));
